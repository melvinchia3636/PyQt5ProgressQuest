...
class ListBox:
  ...
  def AddUI(self, caption): pass

  def ClearSelection(self): pass

  def PutUI(self, key, value): pass

  def scrollToTop(self): pass

  def rows(self): pass

  def CheckAll(self, butlast=None): pass
...
def FormCreate():
  ...
  ExpBar = ProgressBar("ExpBar", "$remaining XP needed for next level")
  EncumBar = ProgressBar("EncumBar", "$position/$max cubits")
  PlotBar = ProgressBar("PlotBar", "$time remaining")
  QuestBar = ProgressBar("QuestBar", "$percent% complete")
  TaskBar = ProgressBar("TaskBar", "$percent%")

  AllBars = [ExpBar, PlotBar, TaskBar, QuestBar, EncumBar]

  Traits = ListBox("Traits", 2, K["Traits"])
  Stats = ListBox("Stats", 2, K["Stats"])
  Spells = ListBox("Spells", 2)
  Equips = ListBox("Equips", 2, K["Equips"])
  Inventory = ListBox("Inventory", 2)
  Plots = ListBox("Plots", 1)
  Quests = ListBox("Quests", 1)
...
def Timer1Timer():
  ...
  json.dump(game, open("sus.json", "w"), indent=4)
...
def StartTimer():
  global timerid, lasttick

  if not timerid:
    lasttick = timeGetTime()
    timerid = Timer(0.1, Timer1Timer)
    timerid.start()
